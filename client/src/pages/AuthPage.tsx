  ...
  return (
    <div className="grid min-h-screen bg-mist lg:grid-cols-[1fr_480px]">
      <section className="hidden border-r border-line bg-ink px-14 py-16 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="grid max-w-2xl grid-cols-3 gap-4">
          {["Admin controls", "Member updates", "Overdue tracking"].map((item) => (
            <div
              key={item}
              className="rounded-lg border border-white/15 bg-white/5 p-4 text-sm text-slate-200 transition-all hover:-translate-y-0.5 hover:border-teal-200/40 hover:bg-white/10"
            >
              {item}
            </div>
          ))}
      ...
      <section className="flex items-center justify-center px-5 py-10">
        <form onSubmit={onSubmit} className="w-full max-w-md animate-fade-in-up rounded-lg border border-line bg-white p-7 shadow-soft">
          ...
          {error && <p className="mt-4 animate-fade-in rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}

          <button
            disabled={submitting}
            className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-md bg-ink px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <button
            ...
            className="mt-4 w-full text-sm font-medium text-teal transition-colors hover:text-teal-700 hover:underline"
          >
